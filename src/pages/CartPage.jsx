...
export default function CartPage() {
  ...
  const handleQuantityChange = async (item, newQty) => {
    ...
    if (newQty > availableStock && userId) {
      // Set error for this specific item (only for logged-in users with real stock data)
      setStockErrors(prev => ({
        ...prev,
        [item.id]: `Exceeding available stock (${availableStock} available)`
      }));
      return;
    }
  ...
  const handleCheckout = () => {
    ...
    if (!userId) {
      showToast("Please login to place your order", "warning", 3000);
      setTimeout(() => {
        navigate("/login");
      }, 1500);
      return;
